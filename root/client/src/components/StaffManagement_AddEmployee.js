...
export default function AddEmployee() {
        ...
        switch (name) {
            case "eid":
                if (!/^[EARS]\d+$/.test(value.toUpperCase())) {
                    errorMessage = "Invalid Employee ID. Please start with E, A, R, or S followed by digits.";
                }
                break;
            case "name":
                if (!/^[a-zA-Z\s]*$/.test(value)) {
                    errorMessage = "Invalid Employee Name. Only letters and spaces are allowed.";
                }
                break;
            case "age":
                if (!/^\d+$/.test(value) || parseInt(value) <= 0) {
                    errorMessage = "Invalid Age. Please enter a positive number.";
                }
                break;
            case "contactno":
                if (!/^\+?\d{10}$/.test(value)) {
    ...
    function sendData(e) {
        ...
        // Validation part
        const formErrors = {};
        for (let field of ["eid", "name", "age", "contactno", "email"]) {
    ...
    return (
        <div style={{ backgroundColor: "white", minHeight: "100vh", display: "flex", justifyContent: "center", alignItems: "center" }}>
            <form onSubmit={sendData} className="mx-auto w-50 shadow p-5" style={{ border: "2px solid #091f36", backgroundColor: "#f8f9fa", color: "#000000" }}>
                ...
                <div className="mb-3">
                    ...
                    <input type="text" className="form-control" id="employeeid" placeholder="Enter Employee ID"
                        name="eid" value={eid}
                        onChange={(e) => { setEid(e.target.value); handleInputChange("eid", e.target.value); }}
                        style={{ backgroundColor: "#ffffff", color: "#0f2862" }}
                    />
                </div>
                <div className="mb-3">
                ...
                <div className="row mb-3">
                    <div className="col-md-6">
                        ...
                        <input type="text" className="form-control" id="age" placeholder="Enter Age"
                            name="age" value={age}
                            onChange={(e) => { setAge(e.target.value); handleInputChange("age", e.target.value); }}
                            style={{ backgroundColor: "#ffffff", color: "#0f2862" }}
                        />
                    </div>
                    <div className="col-md-6">
                ...
                <div className="mb-3">
                    ...
                    <input type="text" className="form-control" id="contactno" placeholder="Enter Contact no"
                        ...
                        onChange={(e) => { setContact(e.target.value); handleInputChange("contactno", e.target.value); }}
                        style={{ backgroundColor:"#ffffff" , color: "#0f2862" }}
