...
export default function EditDamageEquip() {
  ...
  function sendData(e) {
    ...
    const updatedDamageEquipment = {
      ...
      damagediscription: damage.damagediscription,
      suppliername :damage.suppliername,
      supplieremail: damage.supplieremail,
    ...
    axios.put(`http://localhost:4000/damageEquipment/editDamageEquipment/${damageId}`, updatedDamageEquipment)
      .then(() => {
        ...
        window.location.href = "/supplier/damage";
      })
      .catch((err) => {
        alert(err);
      });
    ...
    <div className="container mt-5">
      <form className="mx-auto w-50 shadow p-5" onSubmit={sendData}>
        ...
        <div className="mb-3">
          ...
          <input type="text" className="form-control" id="discription" placeholder="Enter Damage Description" name="damagediscription" value={damage.damagediscription} onChange={(e) => setDamage({ ...damage, damagediscription: e.target.value })} required />
        </div>
        <div className="mb-3">
          <label htmlFor="quantity_required" className="form-label">Supplier Name</label>
          <input type="text" className="form-control" id="quantity_required" placeholder="Enter Technician Name" name="technicianname" value={damage.suppliername} onChange={(e) => setDamage({ ...damage, suppliername: e.target.value })} required />
        </div>
        <div className="mb-3">
        ...
        <div className="mb-3">
          ...
          <input type="text" className="form-control" id="purchase_to_be_made" placeholder="Enter Technician Email" name="technicianemail" value={damage.supplieremail} onChange={(e) => setDamage({ ...damage, supplieremail: e.target.value })} required />
        </div>
        <div className="mb-3">
          <label htmlFor="purchase_to_be_made" className="form-label">Technician Email</label>
          <input type="text" className="form-control" id="purchase_to_be_made" placeholder="Enter Technician Email" name="technicianemail" value={damage.technicianemail} onChange={(e) => setDamage({ ...damage, technicianemail: e.target.value })} required />
        </div>
        <div className="mb-3">
