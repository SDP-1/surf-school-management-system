  ...
  const handleSearch = (e) => {
    ...
    setFilteredDamageEquipment(
      damageEquipment.filter((item) =>
        ...
        item.technicianname?.toLowerCase().includes(searchTermLower))
      )
    );
  };
...
doc.addImage(logo, "PNG", doc.internal.pageSize.width - 40, 5, 50, 50);
    ...
    doc.autoTable({
      head: [['Damage Id','Equipment No', 'Equipment Name', 'Damage Discription', 'Technician Name', 'Technician Email', 'Assigned date', 'Delivery date', 'Repair Cost']],
      body: filteredDamageEquipment.map((damageEquipment, index) => [damageEquipment.damageId,damageEquipment.equipmentno, damageEquipment.equipmentname, damageEquipment.damagedescription, damageEquipment.technicianname, damageEquipment.technicianemail,new Date(damageEquipment.assigneddate).toLocaleDateString() , new Date(damageEquipment.deliverydate).toLocaleDateString(), damageEquipment.repaircost]),
          ...
          <tbody>
            {filteredDamageEquipment.map((damageEquipment, index) => (
              <tr key={index}>
                ...
                <td>{damageEquipment.damagedescription}</td>
