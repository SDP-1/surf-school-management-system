...
function AttendanceManager() {
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        <div style={{ flex: 1, marginRight: '20px', backgroundColor: '#f8f9fa', padding: '20px', borderRadius: '10px', boxShadow: '0 0 10px rgba(0, 0, 0, 0.5)' }}>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <button type="submit" style={{ backgroundColor: '#87CEFA', border: 'none', color: '#ffffff', padding: '10px 20px', borderRadius: '5px', cursor: 'pointer' }}>Add Attendance</button>
            </form>
        </div>
    ...
    <div style={{ marginTop: '20px', backgroundColor: '#f8f9fa', padding: '20px', borderRadius: '10px', boxShadow: '0 0 10px rgba(0, 0, 0, 0.5)' }}>
        ...
        <input
            ...
            style={{ marginBottom: '10px', width: '100%', padding: '8px', borderRadius: '5px', border: '1px solid #000000' }}
        />
        {isLoading ? (
            <p>Loading...</p>
        ) : error ? (
            ...
            <table id="attendance-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                    <tr>
                        <th style={{ border: '1px solid #000000', textAlign: 'left', padding: '8px' }}>Employee ID</th>
                        <th style={{ border: '1px solid #000000', textAlign: 'left', padding: '8px' }}>Name</th>
                        <th style={{ border: '1px solid #000000', textAlign: 'left', padding: '8px' }}>Date</th>
                        <th style={{ border: '1px solid #000000', textAlign: 'left', padding: '8px' }}>Actions</th>
                    </tr>
                </thead>
                <tbody>
                    {filteredAttendances.map((attendance) => (
                        <tr key={attendance._id} style={attendance.eid === searchEmployeeId ? { backgroundColor: '#87CEFA' } : {}}>
                            <td style={{ border: '1px solid #000000', textAlign: 'left', padding: '8px' }}>{attendance.eid}</td>
                            <td style={{ border: '1px solid #000000', textAlign: 'left', padding: '8px' }}>{attendance.name}</td>
                            <td style={{ border: '1px solid #000000', textAlign: 'left', padding: '8px' }}>{attendance.date}</td>
                            <td style={{ border: '1px solid #000000', textAlign: 'left', padding: '8px' }}>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0, 0, 0, 0.5)', zIndex: 9999 }}>
            <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', backgroundColor: '#ffffff', padding: '20px', borderRadius: '5px', boxShadow: '0 0 10px rgba(0, 0, 0, 0.5)' }}>
                ...
                <form onSubmit={handleUpdate}>
                    ...
                    <button type="submit" style={{ backgroundColor: '#87CEFA', border: 'none', color: '#ffffff', padding: '10px 20px', borderRadius: '5px', cursor: 'pointer' }}>Update</button>
    ...
    <div style={{ marginTop: '20px' }}>
        <button onClick={generateReport} style={{ backgroundColor: '#87CEFA', border: 'none', color: '#ffffff', padding: '10px 20px', borderRadius: '5px', cursor: 'pointer' }}>Generate Report (All)</button>
        ...
        <button onClick={() => generateEmployeeReport(employeeIdForCount)} style={{ backgroundColor: '#87CEFA', border: 'none', color: '#ffffff', padding: '10px 20px', borderRadius: '5px', cursor: 'pointer' }}>Generate Report (Employee)</button>
    </div>
</div>
